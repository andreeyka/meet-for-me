...
# `private import ...`), блочные комментарии в шапке (`/* c */ import ...`,
# `@_spi(A) /* c */ import ...`), необязательное ключевое слово вида импорта
...
# legal, скобка сама служит границей токена), модификатор доступа и
# комментарии в шапке — возвратом РП после #150.
#
...
#
# Блочный комментарий — `/\*(?:(?!\*/).)*\*/`, а НЕ нежадный `/\*.*?\*/`
# (возврат РП после #152): нежадность — только предпочтение движка при
# первой попытке, не гарантия. Если после первого найденного `*/` остаток
# строки не даёт всей регулярке дойти до `$`, движок откатывается и
# раздвигает `.*?` до следующего `*/` дальше по строке — и тогда
# `import Foundation; /* x */ import EventKit /* y */` целиком читается
# как один комментарий `/* x */ import EventKit /* y */`, а спрятанный
# внутри второй `import EventKit` пропадает молча. Отрицательный
# опережающий просмотр `(?!\*/)` перед каждым символом тела запрещает
# сам вход внутрь любого `*/`, поэтому у комментария физически нет
# отступать некуда — он обязан кончиться на первом же `*/`.
_BLOCK_COMMENT = r"/\*(?:(?!\*/).)*\*/"
_CMT = r"(?:" + _BLOCK_COMMENT + r"\s*)*"
_ATTR = r"(?:@\w+\([^)]*\)\s*|@\w+\s+)*"
_ACCESS = r"(?:(?:public|package|internal|fileprivate|private)\s+)?"
_KIND = r"(?:(?:struct|class|enum|protocol|func|var|let|typealias)\s+)?"
_TAIL = r"\s*;?\s*" + _CMT + r"(?://.*)?$"
IMPORT_RE = re.compile(
    r"^\s*" + _CMT + _ATTR + _CMT + _ACCESS + _CMT + r"import\s+" + _KIND + r"([A-Za-z0-9_.]+)" + _TAIL
)
...
LOOSE_IMPORT_RE = re.compile(r"^\s*" + _CMT + _ATTR + _CMT + _ACCESS + _CMT + r"import\b")
...
def self_test():
    import_cases = [
        ...
        (
            "import Foundation; /* x */ import EventKit /* y */\n",
            [
                "строка похожа на import, но не разобрана целиком: "
                "import Foundation; /* x */ import EventKit /* y */"
            ],
        ),  # РП, возврат после #152: бэктрекинг нежадного /\*.*?\*/ прятал второй import внутри «комментария»
        (
            "import Foundation /* a */; import EventKit /* b */\n",
            [
                "строка похожа на import, но не разобрана целиком: "
                "import Foundation /* a */; import EventKit /* b */"
            ],
        ),  # РП: та же уязвимость, комментарий до `;`
        ("/* c */ import EventKit\n", ["import EventKit"]),  # РП: комментарий в шапке, перед всем остальным
        ("/* c */ import Foundation\n", []),  # то же, разрешённый модуль
        ("@_spi(A) /* c */ import EventKit\n", ["import EventKit"]),  # РП: комментарий между атрибутом и import
    ]
    failures = 0
