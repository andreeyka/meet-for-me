...
называет теперь оба имени решения, `IR-003` и приёмку MEE-26 (находка 5.2 MEE-211); карточки
`domain-core` и `detector` приведены к действующему изданию C-009 — v7 вместо v6 в трёх ссылках.
...
- Запрещено: импорт AppKit, SwiftUI, AVFoundation, CoreAudio, EventKit, GRDB, XPC. Только Foundation.
  ...
  выходят значения, а не путь (C-009 v7 §6, инвариант 23). Всё остальное, что лежит в файловой системе, —
...
- Чем не закрыта — **четыре вещи; счёт мой, проверенный по дереву, а не перенесённый из запроса**:
  ...
  2. требование «у модуля-реализатора обязан быть хотя бы один собственный публичный тип» (инвариант 10
     C-007 v6, инвариант 25 C-004 v3, инвариант 21 C-009 v7, инвариант 19 C-015 v3) у `domain-core`
...
- Потребляет контракты: DTO домена; таблицы правил — данные в ресурсах собранного модуля, не код.
  В чьих ресурсах лежит таблица, называет её контракт: по C-009 v7 §6 `providers.json` и `clients.json` —
