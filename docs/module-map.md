...
Изменение против v1.8: третий возврат РП по IR-118 (MEE-348) — запись v1.8 (абзац ниже) назвала
составным корнем `C-016`. Это неверно: `C-016` — фасад приложения для UI (см. таблицу контрактов
и раздел «МОДУЛЬ: app-ui» ниже), составной корень — сам `app-ui` («App-таргет… composition root»,
«Реализации связываются в composition root (`app-ui`)»). Строка `calendar-eventkit` правлена: `PlatformResolver`
инжектируется составным корнем `app-ui`, не `C-016`. Текст абзаца v1.8 ниже сохранён для истории с
этой оговоркой, а не переписан.
...
**Поправка v1.9: составной корень — `app-ui`, не `C-016`** (см. абзац выше). Живая строка модуля
`calendar-eventkit` ниже (раздел «МОДУЛЬ: calendar-eventkit») `C-016` не называла ни разу — ошибка
была только в тексте этого абзаца, править в определении модуля нечего.
