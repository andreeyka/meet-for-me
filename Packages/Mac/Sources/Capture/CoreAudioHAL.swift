...
enum HALTap {
    ...
    // СТРОКА: догадка признака отказа по праву (см. шапку файла) — единственный код, который
    // спайк вообще наблюдал у Core Audio для отказа TCC (при перечислении процессов); контракт
    // сам называет это неизмеренным (§«Опора на приватный API…», п. 4), подтвердит только М1.
...
// СТРОКА: C-004 §«Опора на приватный API…», п. 1 называет этот контракт зависимым от
// `responsibility_get_pid_responsible_for_pid` «через вход», не самим вызывающим его напрямую —
// формулировка читается так, что ответственный bundle id мог бы приходить в `Capture` готовым
// значением (например, снятым слоем, которому уже разрешено звать приватный API, как это делает
// `Detector.ProcessIdentity` для своей обязанности), а не вычисляться внутри этого модуля второй
// раз. Ниже — вызов самим модулем: тот же приём и то же обоснование деградации, что у
// `Detector.ProcessIdentity`, продублированный здесь, а не переиспользованный оттуда, потому что
// `Capture` не вправе зависеть от `Detector` (граница модуля, `Package.swift` вне зоны задачи) —
// а входа, которым ответственный bundle id мог бы прийти готовым, в `CaptureRequest`/`HardwareEvent`
// сегодня нет. Вилка не решена мной: «вызывать самому» (ниже) против «получать через вход»
// (потребовал бы новое поле контрактных типов, вне зоны трёх директорий задачи) — решение за РП.
///
/// C-009 §4.1, шаг 1 (`responsibleBundleId`): ответственный процесс разрешается приватным
/// символом `responsibility_get_pid_responsible_for_pid` через `dlsym`. Деградация: символ может
/// не разрешиться на будущей macOS — тогда `bundleId(ofPid:)` возвращает `nil` и шаг 1
/// сворачивается к обычному `bundleId`.
