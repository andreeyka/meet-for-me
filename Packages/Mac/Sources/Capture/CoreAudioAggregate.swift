...
//
//  IR-114 (MEE-332) закрыт архитектором изданием C-004 v6 (MEE-77) — инвариант 6 дословно:
//  «Компенсация дрейфа включена всегда, кроме опорного элемента. Она включается при каждой
//  сборке aggregate device на каждом саб-устройстве и tap-е, кроме одного — того, который эта же
//  сборка назначает опорными часами (`kAudioAggregateDeviceMainSubDeviceKey`, master clock): у
//  него компенсация физического смысла не имеет… и она выключена безусловно, отдельным
//  основанием, а не тем же „выключить нечем“». Ниже (`buildComposition`/`assembleComposition`)
//  опорный элемент держит `kAudioSubDeviceDriftCompensationKey`/`kAudioSubTapDriftCompensationKey:
//  0` БЕЗУСЛОВНО, независимо от `driftCompensation` — это вариант (а) прежней вилки, принятый
//  архитектором текстом самого инварианта; правка кода не потребовалась.
//
...
final class AggregateRuntime: @unchecked Sendable {
    ...
    /// у самого опорного — `0` безусловно (инвариант 6, C-004 v6, см. шапку файла).
    ///
