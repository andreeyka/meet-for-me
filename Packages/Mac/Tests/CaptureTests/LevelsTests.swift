...
final class LevelsTests: CaptureAsyncTestCase {
    ...
    func test_k25_levelsThrottledByDeliveryTimeNotByBufferHostTime() async throws {
        ...
        // событий не обязана иметь интервал меньше окна троттлинга минус запас на планировщик
        // (80 мс при окне 100 мс). Это ловит и «без троттлинга вовсе» (интервалы внутри пачки —
        // доли миллисекунды), и троттлинг «по счётчику, не по времени» (интервалы внутри пачки
        // тоже остались бы почти нулевыми, даже если итоговый счёт выглядит разумным).
        for pairIndex in 1..<levels.count {
            ...
            XCTAssertGreaterThanOrEqual(gapMs, 80,
                                        "интервал между доставленными событиями #\(pairIndex - 1) и #\(pairIndex) "
                                        + "обязан быть не меньше окна троттлинга (100 мс), а не \(gapMs) мс")
        }
