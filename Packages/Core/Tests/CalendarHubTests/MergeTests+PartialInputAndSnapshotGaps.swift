...
extension MergeTests {
    ...
    func test_k78_winnerWithoutSnapshotSkipsToFirstSnapshotHolderByAscendingSourceId() async throws {
        ...
        // Возврат РП (24.09, приёмка #119, 22:12 UTC, п. 2): шаг 5 ответа К78 — lastModified
        // результата берётся у ПОБЕДИТЕЛЯ шага 1 (z-conn), независимо от того, что у него нет
        // снимка — та же честь, что у identity/сортировки, не завязана на наличие payload.
        XCTAssertEqual(
            stored.event.lastModified, base.addingTimeInterval(10),
            "lastModified результата — победителя шага 1 (z-conn), а не источника, давшего содержимое"
        )
    }
}
