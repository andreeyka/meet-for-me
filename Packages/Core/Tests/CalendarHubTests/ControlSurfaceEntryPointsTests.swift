...
final class ControlSurfaceEntryPointsTests: XCTestCase {
    ...
    // Два пограничных случая отмены из #94 (бэклог MEE-386) + generation-guard на
    // setSyncOutcome («часть 3в»/«часть 3г») — `SyncOneCancellationDefectsTests.swift`
    // (SwiftLint `file_length` считает каждый файл отдельно, этот файл вышел за лимит).
}
