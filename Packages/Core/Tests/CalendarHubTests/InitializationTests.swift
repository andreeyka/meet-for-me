...
//    MAJOR-версией `protocolVersion` нужен `ScriptedRPCTransport` (stdio, группа Ж — теперь
//    написана, МЕЕ-402 шаг 2) — `FakeCalendarConnector` не проходит через кадры протокола
//    вовсе; сам тест — следующая часть MEE-386.
//  * К9 — только вход А (таймаут через зависший вызов). Вход Б (кадр `shutdown` на
//    stdio-пути) — та же следующая часть.
//
