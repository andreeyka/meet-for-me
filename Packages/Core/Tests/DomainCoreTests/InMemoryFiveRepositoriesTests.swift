...
extension InMemoryFiveRepositoriesTests {
    ...
    /// и идентификаторе; прочие вызовы работают как прежде; отказ снимается.
...
extension InMemoryFiveRepositoriesTests {
    ...
    func test_mee320_speakerProfileRepository_failsOnNamedMethodAndId() async throws {
        ...
        let corrupted = StorageError.dataCorrupted(
            entity: "SpeakerProfile", id: personId.uuidString, message: "инвариант 11, длина не совпала"
        )
        repositories.speakerProfiles.fail(with: corrupted, on: .profile, id: personId.uuidString)
