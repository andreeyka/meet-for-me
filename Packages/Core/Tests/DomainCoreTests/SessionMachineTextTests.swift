...
final class SessionMachineTextTests: XCTestCase {
    ...
    /// Машина не читает снимка прав: ноль вхождений четырёх имён.
    ///
    /// ОБЛАСТЬ ПУТЕВАЯ, И ШАПКА ГОВОРИТ ТО ЖЕ, ЧТО ТЕЛО. Прежняя редакция этой строки
    /// утверждала «область СПЛОШНАЯ», тогда как фильтр ниже брал пути `SessionMachine*` и
    /// `SessionCoordinator.swift`, — два ответа на один вопрос в одном тесте (приёмка
    /// MEE-301 §7 №2; дельта `Щ` §3, строка 6). Верно тело: действующий текст К54 называет
    /// область путевой в самом Входе — «та же, что у К19 и К27», — и называет прямо, почему:
    /// сплошная КРАСИТ ВЕРНУЮ РЕАЛИЗАЦИЮ. Замер аналитика, `main@4accdaa`: сплошным
    /// прогоном по `Sources/DomainCore/` четыре имени дают СЕМЬ вхождений — три
    /// `PermissionSnapshot` и три `PermissionsPort` в `PermissionsPort.swift` (объявление
    /// порта C-007) и одно в шапке `DomainCore.swift`; путевым — ноль по всем четырём.
    /// Пункт запрещает машине ЧИТАТЬ снимок прав, а не модулю ИМЕТЬ объявление чужого порта.
    ///
    /// **Колонка «Чем проверяется» плана MEE-288 по-прежнему называет область сплошной, и
    /// это чужая строка, которой я не трогал:** владелец — QA, дельта `Щ` §3 строка 4,
    /// условие снятия — колонка приводится к Входу пункта, срок — до выдачи части C.
