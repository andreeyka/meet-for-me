...
//  требует прямо. Файлами они не лежат, и опора у этой формы — та, что стоит в действующей
//  редакции п. 14: эталон обязан меняться вместе с изданием контракта, который он описывает,
//  а одноимённые файлы-ресурсы в `Fixtures/` тестовый таргет править не вправе — каталог
//  принадлежит архитектору (`Packages/Core/Package.swift:43`, решение по IR-003), и правка
//  файла там есть `interface-request`, а не коммит.
//
//  Прежняя опора снята как ложная, и снята прогоном: `resources:` тестового таргета в этом
//  же `Package.swift` как раз ОБЪЯВЛЕНЫ — строка 47, `resources: [.copy("Fixtures")]`.
//
//  Про сами файлы каталога — по факту, а не шире факта: `Fixtures/manifest.json` описывает
//  C-002 v2 и действующим изданием не читается (ключа `discontinuities` в нём нет вовсе),
//  а `Fixtures/transcript.v1.json` СЕГОДНЯ ЧИТАЕТСЯ: версия схемы транскрипта не повышалась.
//  Он станет тем же манифестом в первое же издание C-003, которое её повысит, — и починить
//  его тестовый таргет будет не вправе.
//
