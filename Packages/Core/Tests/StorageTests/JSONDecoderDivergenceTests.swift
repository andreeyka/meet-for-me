...
//  на всех входах, отказ сведён к `StorageError.dataCorrupted`. Поля с
...
//  - `connectors.selected_calendar_ids_json` (`[String]`) — плоский массив
//    без объектных ключей и без числовых полей вовсе: ни один из трёх
//    входов буквально не применим. Строка для аналитика — не решаю сама.
...
final class JSONDecoderDivergenceTests: StorageAsyncTestCase {
    ...
    func testK43_manifestJSONRejectsFractionalSchemaVersion() async throws {
        try await Self.assertManifestRejected(schemaVersionText: "3.5")
    }
    ...
    func testK43_manifestJSONRejectsDuplicateTopLevelKey() async throws {
        ...
        try await Self.assertDataCorrupted { try await recordings.recording(id: recordingId) }
    }
    ...
    private static func assertWordsJSONRejected(_ brokenWordsJSON: String) async throws {
        ...
        try await Self.assertDataCorrupted { try await transcripts.transcript(id: header.id) }
    }
    ...
    func testK43_payloadJSONRejectsDuplicateTopLevelKey() async throws {
        ...
        do {
            _ = try await jobs.job(id: job.id)
            XCTFail("ожидался dataCorrupted")
        } catch let error as StorageError {
            guard case .dataCorrupted = error else { XCTFail("ожидался dataCorrupted, получено \(error)"); return }
        }
    }
