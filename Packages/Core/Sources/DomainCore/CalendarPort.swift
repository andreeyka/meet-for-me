...
public enum DedupKey: Hashable, Codable, Sendable {
    ...
    // IR-123 (MEE-359) закрыт архитектором изданием C-005 v9: инвариант 3 разрешил
    // собственное расхождение формулы (`Int(x / 60) * 60` — усечение к нулю) и текста
    // («округление ВНИЗ до минуты») в пользу floor — тот же выбор, что стоял здесь
    // временно (MEE-352), теперь действующий текст контракта, а не решение реализатора.
