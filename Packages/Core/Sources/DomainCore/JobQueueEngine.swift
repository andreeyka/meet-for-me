...
public actor JobQueueEngine: JobQueue {
    ...
    /// Число НЕЗАКРЫТЫХ обязательств «где-то ещё предстоит пересмотр» — считает не только
    /// вызванные явно (`start()`, `submit()`), но и то, что заведёт на себя завершение задачи
    /// (`executeAndFinish`, конец §7): обязательство заводится уже в момент ПОСТАНОВКИ задачи
    /// обработчику (`beginExecuting`), а не в момент, когда `executeAndFinish` фактически
    /// позвонит `runRevisitPass()` — см. довод у `beginExecuting`. MEE-363: `waitUntilIdle()`
    /// без этого счётчика возвращался, пока такой пересмотр УЖЕ занял кандидата без
    /// обработчика (`claimNext`), но ЕЩЁ НЕ вернул его строку в `pending` (`noHandler`, §7) —
    /// прямое чтение репозитория в этот миг ловило `running` вместо ожидаемого `pending`.
    var activeRevisitPasses = 0
    /// MEE-363: настоящий цикл `claimNext` идёт не более чем один одновременно — второй
    /// вызов `runRevisitPass()`, пришедшийся на время, пока первый ещё не домотал свой
    /// `while`, не заводит СОБСТВЕННЫЙ параллельный цикл (тот гонялся бы за тем же
    /// `claimNext` одновременно с первым — оба видят одну и ту же СТРОКУ то занятой, то уже
    /// отпущенной другим, и годятся оба заново её взять), а просит текущий цикл сделать ещё
    /// один заход после того, как домотает этот. См. `runRevisitPass()`.
    var isRevisitLoopRunning = false
    /// Заявка «сделать ещё один заход», оставленная вызовом `runRevisitPass()`, пришедшимся
    ...
    /// переживают сам актор: неструктурная `Task` не отменяется сама по себе от того, что
    /// объект, её заведший, освобождён — `[weak self]` внутри нужен телу задачи (не звать
    /// метод на уже несуществующем акторе), но САМУ задачу это не останавливает, вызвать
    /// `cancel()` обязана здесь именно эта `deinit`.
    ///
    /// MEE-378, п. 8 (возврат РП после приёмки #93, поправка находки MEE-375): прежний текст
    /// здесь ошибочно утверждал, что `cancel()` не завершает `for await` над `AsyncStream` без
    /// явного `finish()`, и что это копится в «сотни зависших задач», останавливающих
    /// планировщик. Это неверно — `cancel()` штатно завершает такую итерацию (`next()`
    /// возвращает `nil` по отмене), тем же приёмом, что `Task.sleep` отвечает `throw`; вывод
    /// про накопление зависших задач опирался на эту ошибку и снят вместе с ней.
