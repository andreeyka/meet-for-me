...
//
//  НЕ РЕШЕНО ЭТИМ ФАЙЛОМ: `TranscriptionJobSpec.wantWordTimestamps`/`.diarizeSystemChannel` —
//  `JobPayload.transcribe` (C-013 §1) несёт только `recordingId`/`profileId`/`language`, три
//  поля из пяти; оставшиеся два здесь заданы `true` (голая заглушка, не решение контракта) —
//  ни один из критериев К43–К45/К47 (предмет этой задачи) их не проверяет, поскольку они
//  проверяют ОТОБРАЖЕНИЕ уже брошенной `TranscriptionServiceError`, не форму отправленного
//  `TranscriptionJobSpec`. Источник этих двух полей (профиль C-014? отдельная задача
//  постановки?) — вопрос, который эта задача не решает; найдено, не спрятано.
//
