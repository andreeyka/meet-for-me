...
//
//  `reclaimExpiredLeases` ПРИМЕНЯЕТ РАЗВИЛКУ ИНВАРИАНТОВ 10/11 ЦЕЛИКОМ, А НЕ ЧАСТЬ ЕЁ: сама
//  формулировка инварианта 11 — «тем же правилом и с той же развилкой, что в инварианте
//  10» — и раздел «Чем проверяется» C-013 явно испытывает МЕТОД `reclaimExpiredLeases`
//  входами «с отметкой — `attempts + 1`, без отметки — прежний», то есть развилку исполняет
//  сам порт, а не вызывающая сторона. Задержка повтора при переходе в `pending` — формула
//  §5 контракта («Прерванная попытка считается тем же правилом»): `min(30 * 2^(attempts_new
//  - 1), 1800)` секунд, только когда попытка действительно начиналась (`attemptStartedAt !=
//  nil`) — иначе `runAfter` не трогается: работы не было, откатывать нечего.
//
