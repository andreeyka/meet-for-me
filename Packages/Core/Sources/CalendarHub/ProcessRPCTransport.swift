...
public actor ProcessRPCTransport: RPCTransport {
    ...
    /// macOS, C-006 §7): `SIGTERM` реального дочернего процесса, ещё не вышедшего по EOF
    /// `stdin` за грейс-период, там НЕ убивает его вовсе — убивает только `SIGKILL` (эскалация
    /// ниже). Четыре версии причины проверены и опровергнуты по факту прогона CI, не оставлены
    /// гипотезой: (1) утечка родительской копии конца канала (`closeParentSideOfPipes`, см.
    /// `init`) — исправлена отдельно, симптом остался; (2) снятие `readabilityHandler` в этом
    /// методе ДО `terminate()` — перенесено на после `waitForExit()`, симптом остался; (3)
    /// маска заблокированных/игнорируемых сигналов ребёнка, наследуемая через `exec` —
    /// проверено печатью `/proc/self/status` изнутри ребёнка, SIGTERM там НЕ заблокирован и НЕ
    /// игнорируется; (4) путь запуска (`env cat` vs `sh -c "exec cat"`) — тоже не при чём, оба
    /// одинаково медленные. `terminationHandler`/EOF при этом срабатывают МГНОВЕННО на
    /// настоящий сигнал что на macOS, что на Linux (после реального `SIGKILL`) —
    /// детектирование смерти процесса в этом файле работает верно на обеих платформах; сама
    /// доставка `SIGTERM` живому ребёнку — свойство конкретного контейнера CI, не этого
    /// транспорта. Сторож — конечная подстраховка, не политика тайм-аутов «Поведения» C-006
    /// §5.2, которую и запрещает «не на стенных часах»; корректность не страдает от его
    /// присутствия — только (на этом конкретном CI, для процесса, что сам не вышел по EOF)
    /// латентность порядка 5с, которой в проде на macOS нет.
