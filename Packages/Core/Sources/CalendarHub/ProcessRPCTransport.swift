...
public actor ProcessRPCTransport: RPCTransport {
    ...
    /// продолжение той же идеи «больше кадров не будет»), но и то, что фактически устранило
    /// известную задержку ~5с на Linux CI (`swift:5.10-jammy`): расследование (возврат РП,
    /// повторная приёмка PR #138) четырежды проверило и опровергло версию причины «в чём-то,
    /// что можно починить в самом сигнальном пути» — утечка конца канала, снятие
    /// `readabilityHandler` до `terminate()`, маска заблокированных сигналов ребёнка (проверено
    /// печатью `/proc/self/status`), путь запуска (`env cat` vs `sh -c "exec cat"`) — во всех
    /// случаях `SIGTERM` живого `cat`/`sh` на этом CI-контейнере не убивал его вовсе, убивал
    /// только `SIGKILL`, при абсолютно мгновенном `terminationHandler`/EOF на настоящий сигнал.
    /// Обход, не устранение причины: раз процесс, читающий до EOF, всё равно выходит сам
    /// (естественный `exit()`, без единого сигнала — путь, уже быстрый и на Linux), `close()`
    /// даёт ему этот шанс ДО того, как вообще понадобится `SIGTERM`. Сторож остаётся конечной
    /// подстраховкой — не политика тайм-аутов «Поведения» C-006 §5.2, которую и запрещает «не
    /// на стенных часах» — на случай плагина, что не выходит по EOF `stdin` и притом (гипотеза
    /// осталась неисчерпанной для НАСТОЯЩИХ плагинов, не тестовых `cat`/`sh`) может повторить ту
    /// же задержку на Linux CI; в проде (только macOS, C-006 §7) `SIGTERM` работает штатно.
