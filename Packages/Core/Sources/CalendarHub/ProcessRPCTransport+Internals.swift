...
extension ProcessRPCTransport {
    ...
    /// `AsyncStream` с одним потребителем), так что никакой ещё не разобранный ОТВЕТ (кадр,
    /// завершённый `\n`) потеряться не может. Незавершённый хвост БЕЗ `\n` (плагин записал
    /// что-то и умер посреди строки, не дописав перевод) на настоящий кадр не тянет —
    /// `extractLine()` требует разделитель, такой хвост тихо остаётся лежать в `buffer`
    /// невостребованным (возврат РП, приёмка PR #146, бэклог) — это осмысленно: неполный кадр
    /// не JSON, отдавать его как ответ было бы хуже, чем отбросить.
